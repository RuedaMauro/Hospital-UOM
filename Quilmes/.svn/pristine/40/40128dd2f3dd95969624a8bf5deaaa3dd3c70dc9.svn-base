...
function Cargar_Titular_Cargado(Resultado) {
    ...
    $("#NombreTitular").html(Paciente.apellido.substring(0, 15) + "...");
    $("#txtcuit").val(Paciente.cuit);
    if (Paciente.RazonSocial != null)
    $("#RazonSocial").html(Paciente.RazonSocial.substring(0, 15) + "...");
...
function Cargar_CUIT_Cargado(Resultado) {
    ...
    {
        $("#RazonSocial").html(Empresas.razonsocial.substring(0,15));
        $("#ControlCUIT").removeClass("error");
        CUIT_ERROR = 0;
    }
    else
