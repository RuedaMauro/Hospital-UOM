...
function Pedidos_Cargados(Resultado) {
    ...
    if (Pedidos != null) {
        ...
        Tabla_Titulo = "<table id='TablaPedidos_div' class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th>IM</th><th>Fecha</th><th>NHC</th><th>Paciente</th><th>Servicio</th></tr></thead><tbody>";
        $.each(Pedidos, function (index, Pedido) {
            Tabla_Datos = Tabla_Datos + "<tr";
            Tabla_Datos = Tabla_Datos + " style='cursor:pointer' onclick=Ventana('CargarIM.aspx?Id=" + Pedido.IM_Id + "');";
            Tabla_Datos = Tabla_Datos + "><td>" + Pedido.IM_Id + "</td><td>" + Pedido.Fecha + "</td><td>" + Pedido.NHC + "</td><td>" + Pedido.Nombre + "</td><td>" + Pedido.Servicio + "</td></tr>";
        });
