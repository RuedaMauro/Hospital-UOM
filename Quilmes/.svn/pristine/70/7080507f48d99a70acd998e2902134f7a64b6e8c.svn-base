...
namespace Hospital
{
    public class FarmaciaBLL
    {
        ...
        public int Insert_Publico_Detalle(Int32 NroPedido, string InsumoId, string Cantidad, string InsumoPrecio, string Descuento, string CANT_UNIDADES, Int32 Contramov,string NroLote)
        {
            ...
            if (Contramov != 1)
            {
                adapter.H2_Insert_Pedidos_Publico_Det(NroPedido, int.Parse(InsumoId), int.Parse(Cantidad), decimal.Parse(InsumoPrecio.Replace(".", ",")), Descuento, int.Parse(CANT_UNIDADES), Convert.ToInt32(((usuarios)HttpContext.Current.Session["Usuario"]).id), NroLote);
            }
            else //Contramovimiento sumo stock y resto total H2_Insert_Pedidos_Publico_Det_SIN_STOCK
            {
                adapter.H2_Insert_Pedidos_Publico_Det(NroPedido, int.Parse(InsumoId), int.Parse(Cantidad), -decimal.Parse(InsumoPrecio.Replace(".", ",")), Descuento, int.Parse(CANT_UNIDADES), Convert.ToInt32(((usuarios)HttpContext.Current.Session["Usuario"]).id), NroLote);
            }
            return NroPedido;
