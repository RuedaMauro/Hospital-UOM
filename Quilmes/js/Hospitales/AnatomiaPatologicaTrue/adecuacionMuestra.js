...
$.ajax({
    ...
    success: function (Resultado) {
        ...
        $.each(lista, function (index, item) {
            var color = "";
            if ((jQuery.inArray(item.id, parent.idsAdecuacion)) >= 0)
                color = "#CEF6CE";
...
//    alert(parent.idsAdecuacion);
    ...
    tecnicasSeparadas = "";
    if ((jQuery.inArray(id, parent.idsAdecuacion)) >= 0) {
        parent.idsAdecuacion.splice(jQuery.inArray(id, parent.idsAdecuacion), 1);
        $("#descripcion" + id).css('background-color', '');
        $.each(parent.adecuacionMuestra, function (index, item) {

          if(item != undefined)
            if (item.toString().toLowerCase().trim() == $("#descripcion" + id).html().toString().toLowerCase().trim()) {
                parent.adecuacionMuestra.splice(index, 1);
            }

        });
        //parent.$("#txtTecnicasEspeciales").val(parent.tecnicas);
        tecnicasSeparadas = parent.adecuacionMuestra.join("   +   ");
        parent.$("#cboMuestraAdecuacion").val(tecnicasSeparadas);
    } else {
        parent.idsAdecuacion.push(id);
        ...
        parent.adecuacionMuestra.push($("#descripcion" + id).html().toUpperCase());
        tecnicasSeparadas = parent.adecuacionMuestra.join("   +   ");
        parent.$("#cboMuestraAdecuacion").val(tecnicasSeparadas);
    }
