...
$("#btnBuscar").click(function () {
    ...
    if (valid) {
        var json = JSON.stringify({ "NHC": $('#txtNHC').val(), "Id": $('#txtNroPed').val(), "Desde": $('#desde').val(), "Hasta": $('#hasta').val(), "ServicioId": $('#cbo_Servicio :selected').val() });
        //alert(json);
        $.ajax({
            type: "POST",
...
function Pedidos_Cargados(Resultado) {
    Pedidos = Resultado.d;
    var Tabla_Datos = "";
    Tabla_Titulo = "<table id='TablaPedidos_div' class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th>Pedido</th><th>NHC</th><th>Paciente</th><th>Servicio</th><th>Usuario</th><th>Fecha</th></tr></thead><tbody>";
    $.each(Pedidos, function (index, Pedido) {
        ...
        Tabla_Datos = Tabla_Datos + "><td>" + Pedido.Pedido_Id + "</td><td>" + Pedido.NHC + "</td><td>" + Pedido.Paciente + "</td><td>" + Pedido.Servicio + "</td><td>" + Pedido.Usuario + "</td><td>" + Pedido.Fecha + "</td></tr>";
    });
