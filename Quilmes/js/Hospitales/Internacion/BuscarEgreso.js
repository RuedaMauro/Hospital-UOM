...
function CargarPagina(Id) {
    var url = "../Internacion/Egreso.aspx?ID_Int=" + Id + "&BusquedaEgr=1";
...
$('#btn_Buscar').click(function () {
    ...
    $.ajax({
        type: "POST",
        url: "../Json/Internaciones/IntSSC.asmx/BuscarInternaciones_Egresos",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Buscar_Completada(Resultado) {
    ...
    Tabla_Titulo = "<table id='TablaInternacionesContenido' class='table table-hover table-condensed'><thead><tr><th>Egreso</th><th>Nro. HC</th><th>Paciente</th><th>Servicio</th><th>Sala</th><th>Cama</th></tr></thead><tbody>";
    $.each(Internaciones, function (index, inter) {
        Tabla_Datos = Tabla_Datos + "<tr ";
        //if (bonos.apellido != "") {
        Tabla_Datos = Tabla_Datos + " onclick=CargarPagina('" + inter.Id + "');";
        //}
        Tabla_Datos = Tabla_Datos + "><td>" + inter.Fecha + "</td><td>" + inter.NHC + "</td><td>" + inter.Paciente + "</td><td>" + inter.Servicio + "</td><td>" + inter.Sala + "</td><td>" + inter.Cama + "</td></tr>";
    });
