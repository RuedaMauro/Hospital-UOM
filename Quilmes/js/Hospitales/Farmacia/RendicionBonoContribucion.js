...
function Usuarios_Lista_Cargada(Resultado) {
    ...
    $.each(Lista, function (index, Usuario) {
        $("#cbo_Usuarios").append($("<option></option>").val(Usuario.id).html(Usuario.Nombre));
    });
}
...
function List_Rendicion_BonoContribucion() {
    ...
    var Usuario = $("#cbo_Usuarios :selected").val();
    //alert(Usuario);
    $.ajax({
        type: "POST",
...
function List_Rendicion_BonoContribucion_Cargada(Resultado) {
    var Lista = Resultado.d;
    var Encabezado = "<table class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th>Paciente</th><th>NHC</th><th>Insumo</th><th>Cantidad</th><th>Total</th><th style='display:none;'>Descuento</th><th>Fecha</th></tr></thead><tbody>";
    var Contenido = "";
...
var frmValid = $("#frm_Main").valid();
    if(frmValid)
    {
        ...
        var f_desde = $("#desde").val();
        var f_hasta = $("#hasta").val();
        var Usuario = $("#cbo_Usuarios option:selected").val();
