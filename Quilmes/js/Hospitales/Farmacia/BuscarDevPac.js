...
function Pedidos_Cargados(Resultado) {
    ...
    if (Pedidos != null) {
        var Tabla_Datos = "";
        Tabla_Titulo = "<table id='TablaPedidos_div' class='table table-hover table-condensed' style='width: 100%;'><thead><tr><th>Nro. Devolución</th><th>NHC</th><th>Paciente</th><th>Servicio</th><th>Fecha</th><th>Usuario</th></tr></thead><tbody>";
        $.each(Pedidos, function (index, Pedido) {
            Tabla_Datos = Tabla_Datos + "<tr";
            Tabla_Datos = Tabla_Datos + " style='cursor:pointer' onclick=Ventana('DevolucionporPaciente.aspx?PedidoId=" + Pedido.Pedido_Id + "');";
