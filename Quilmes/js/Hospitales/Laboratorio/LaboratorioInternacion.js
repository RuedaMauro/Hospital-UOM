...
$("#txtSubCodigo").keypress(function (event) {
    var Numeros = /^([0-9])*$/;
    if (event.which == 13 || event.keyCode == 9) {
        ...
        if (Numeros.test($("#txtSubCodigo").val())) {            
            if ($("#txtSubCodigo").val() == "") {
                 $("#txtPracticaComentario").focus();
            }
            else {
                 var exists = false;
            $('#cbo_SubPracticas option').each(function () {
                if (this.value == $("#txtSubCodigo").val()) {
                    $("#cbo_SubPracticas option[value=" + $("#txtSubCodigo").val() + "]").attr("selected", true);
...
$("#txtPracticaComentario").keypress(function (event) {
    var Numeros = /^([0-9])*$/;
    if (event.which == 13 || event.keyCode == 9) {
    
        event.preventDefault();
        $("#btnAgregarPractica").click();
        }});
...
$("#btnAgregarPractica").click(function () {
    ...
    if ($("#txtCodigo").val() == '' || $("#txt_practica").val() == '') {
        ...
        $("#txt_practica").removeClass("error");
...
if ($("#txt_Bono").val() != "")
{
    var json = JSON.stringify({ "NroBono": $("#txt_Bono").val() });
    $.ajax({
        type: "POST",
        url: "../Json/Bonos/Bonos.asmx/EstadoBonoProvi",
        data: json,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success:
        function (Resultado) {
            ...
            if (!Estado) {
                $('#BonoEstado').attr('src', '../img/Icono_ERROR.gif');
                alert("ATENCIÓN: El Bono ha sido utlizado anteriormente");
                document.location = "../Laboratorio/LaboratorioInternacion.aspx";
            }
            else {
                $("#btnConfirmarNuevoBono").show();
                $('#BonoEstado').attr('src', '../img/Icono_OK.gif');
            }
        }
        ,
        error:
    ...
    if (Impreso == 0) {
        ...
        if (Valido) {
            ...
            var json = JSON.stringify({ "objPracticas": objPracticas,
                ...
                "Servicio": $("#cbo_Servicio :selected").text(),
                "Sala": $("#cbo_Sala :selected").text(),
                "Cama": $("#cbo_Cama :selected").text(),
                "Documento": $('#txt_dni').val(),
                "NroBono": $('#txt_Bono').val()
            });

          
            $.ajax({
                type: "POST",
                url: "../Json/Laboratorio/Laboratorio.asmx/GuardarLaboratorioInternacion",
                data: json,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
    ...
    $.fancybox(
		{
		    ...
		    'onClosed': function () {
		        window.location.href = "LaboratorioInternacion.aspx";
		    }
		});
}
...
function RecargarPagina(url) {
    document.location = "../Laboratorio/LaboratorioInternacion.aspx" + url;
}
...
$("#cbo_Servicio").change(function () {
    $('#cbo_Cama').empty();
    $('#cbo_Cama').append('<option value="0">Seleccione Cama</option>');
...
function CargarSalas_Cargados(Resultado) {
    ...
    $('#cbo_Sala').empty();
    $('#cbo_Sala').append('<option value="0">Seleccione Sala</option>');
...
$("#btnObtenerPreparacion").click(function () {
    Elementos = "";

    $.each(objPracticas, function (index, practica) {
        Elementos = Elementos + (practica.Codigo + ";" + practica.SubPracticaCodigo) + ",";
    });
    ...
    $.fancybox(
		{
		    'autoDimensions': false,
		    'href': '../Impresiones/LaboratorioComprobante.aspx?Practicas=' + Resultado.d,
		    'width': '75%',
		    'height': '75%',
		    'autoScale': false,
		    'transitionIn': 'none',
		    ...
		    'enableEscapeButton': false,
		    'onClosed': function () {
		        window.location.href = "Preparacion.aspx";
		    }
		});
});